/*
Previo 12
Fecha de entrega 3 de Noviembre del 2025
...
typedef struct _frame {
	...
	// NUEVO: animación por parte
	float FLegs;      // patas delanteras
	float FLegsInc;
...
void interpolation(void)
{
	...
	// ARREGLO: calcular incremento correcto para la cabeza
...
void DoMovement()
{
	...
	// ----- CONTROLES DE PARTES -----
	// Patas delanteras: Z / X
	if (keys[GLFW_KEY_Z]) { FLegs += 1.0f; }
	if (keys[GLFW_KEY_X]) { FLegs -= 1.0f; }

	// Patas traseras: C / V
	if (keys[GLFW_KEY_C]) { RLegs += 1.0f; }
	if (keys[GLFW_KEY_V]) { RLegs -= 1.0f; }

	// Cola: B / N
	if (keys[GLFW_KEY_B]) { tail += 1.0f; }
	if (keys[GLFW_KEY_N]) { tail -= 1.0f; }
