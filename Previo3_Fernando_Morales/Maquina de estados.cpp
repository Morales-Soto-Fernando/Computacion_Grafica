/*
Previo 11
Fecha de entrega 27 de octubre del 2025
...
void Animation() {
	...
	if (dogAnim == 1) { // walk Animation
		...
		// Esta es la nueva condición para limitar el movimiento del perro
		if (dogPos.z < 2.0f) { // Límite del borde en el eje Z
			dogPos.z += 0.001; // Movimiento hacia adelante

		}
		else {
			// Detener la animación si el perro ha llegado al límite
			dogAnim = 0; // Detenemos la animación del perro (por ejemplo, con un valor de 0)
			rotDog = 0.0f; // Detenemos cualquier rotación si es necesario
			RLegs = 0.0f;   // Detenemos las piernas
			FLegs = 0.0f;
			head = 0.0f;    // Detenemos la cabeza
			tail = 0.0f;    // Detenemos la cola
		}
	}
}
